...
export function encode(buffer: Uint8Array): Uint8Array {
  ...
  const output = new Uint8Array(
    Math.ceil(Math.ceil((buf.length * 4) / 3) / 4) * 4
  );
  ...
  for (let i = 0; i < continuous; i += 3) {
    ...
    output[k + 1] = BASE64_ENCODE_TABLE.get(
      ((buf[i] & 0x03) << 4) | (buf[i + 1] >> 4)
    );
    output[k + 2] = BASE64_ENCODE_TABLE.get(
      ((buf[i + 1] & 0x0f) << 2) | (buf[i + 2] >> 6)
    );
...
export function decode(buf: Uint8Array): Uint8Array {
  const dec = buf.map((v) => BASE64_DECODE_TABLE.get(v));
